...
@app.route("/products/<int:product_id>", methods=["DELETE"]) # dymanic = int:product_id because if it's not dynamic, it's going to delete everything
def delete_product(product_id):
    # find the product what that id from the db, 
...
def get_categories():
    ...
    data = categories_schema.dump(categories_list) # using marshmallow's schema
    return data
...
def update_category(category_id):
    ...
    # if category exists
    if category:
        # update the attributes with fields from the request body
        category.name = body_data.get("name") or category.name # the latter is for leaving category the same, since it's partial update with PATCH
